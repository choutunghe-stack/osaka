...
const stops=[
 ...
 ['接著','南京町午餐','優先選有座位、冷氣的店家，吃完直接往港區移動。','南京町 神戸','todo'],
...
function patch(){
 ...
 if(summary)summary.innerHTML='<h3>生田神社已完成，北野異人館與神戶港慢遊</h3><p>神戶 · 週五</p>';
 const body=card.querySelector('.day-body');
 if(!body)return;
 body.innerHTML=`<div class="day-rhythm"><span class="pill">✓ 生田神社完成</span><span class="pill">下一站：異人館</span><span class="pill">取消麵包超人館</span></div><div class="timeline">${stops.map(s=>`<div class="stop" data-status="${s[4]}"><div class="stop-time">${s[0]}</div><div class="stop-dot"></div><div class="stop-content"><b>${s[1]}</b><p>${s[2]}</p><div class="stop-links"><a class="mini-link" target="_blank" rel="noopener" href="${mapUrl(s[3])}">📍 地圖</a></div></div></div>`).join('')}</div><div class="day-note">🏠 異人館精華：風見雞之館、萌黃之館、北野町廣場。北野位於坡地，帶推車建議從生田神社搭計程車上山，再往三宮與舊居留地方向下坡移動。</div><div class="day-note">💡 已取消麵包超人兒童博物館。更新後動線：生田神社 → 北野異人館街 → 神戶舊居留地 → 南京町 → 神戶塔 → Meriken Park → Harborland，港區時間更充裕。</div>`;
 card.dataset.kobeUpdated='32';
}
