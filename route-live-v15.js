...
'2026-07-19':{area:'西梅田・梅田',route:['現在：西梅田（已抵達）','KITTE大阪／Herbis 午餐與休息','LUCUA／大阪 Station City','Grand Front 或 Yodobashi Umeda','返回 VESSEL INN NAMBA']},
...
function status(key,i,last){
 ...
 if(key==='2026-07-19')return i===0?'現在':i===last?'收尾':'接著';
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-19'?'7/19 西梅田・梅田｜目前已抵達西梅田，接著以大阪站周邊室內商場為主。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
