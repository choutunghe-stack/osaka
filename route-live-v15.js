...
'2026-07-19':{area:'梅田',route:['✓ 西梅田（已抵達）','✓ 阪急梅田總店','✓ 阪急百貨退稅完成','現在推薦：Yodobashi Umeda 5F 模型／公仔','備選：Mandarake 梅田店 3F TOY','返回 VESSEL INN NAMBA']},
...
function status(key,i,last){
 ...
 if(key==='2026-07-19')return i<3?'完成':i===3?'下一站':i===last?'收尾':'備選';
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-19'?'7/19 梅田｜阪急百貨退稅完成；下一站建議到 Yodobashi Umeda 5F 找獵人與神劍闖江湖公仔。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
