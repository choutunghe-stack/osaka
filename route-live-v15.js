...
function status(key,i,last){
 ...
 if(key==='2026-07-22')return i<2?'完成':i===last?'返飯店':'行程';
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-22'?'7/22 臨空城・大阪｜已去臨空城，之後到 EDION 購買任天堂點數卡與瑪利歐商品。':key==='2026-07-21'?'7/21 大阪・宇治・道頓堀｜實際完成：高島屋、平等院、中村藤吉抹茶、道頓堀與美津の大阪燒。':key==='2026-07-20'?'7/20 心齋橋・難波｜實際完成：心齋橋 PARCO、法善寺、難波千日前商店街與 CoCo壱番屋晚餐。':key==='2026-07-19'?'7/19 梅田・茶屋町｜駿河屋為最後一站，之後直接返回難波飯店。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
 const live=root.querySelector('.v31-live div');
 if(live)live.textContent=`${['2026-07-17','2026-07-18','2026-07-19','2026-07-20','2026-07-21'].includes(key)?'實際完成路線':key==='2026-07-22'?'今日實際路線':'今日路線'}｜${d.route.join(' → ')}。`;
}
