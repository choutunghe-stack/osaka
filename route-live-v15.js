...
'2026-07-20':{area:'心齋橋・難波',route:['✓ 心齋橋 PARCO','✓ 法善寺','✓ 難波千日前商店街','✓ 返回 VESSEL INN NAMBA']},
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-20'?'7/20 心齋橋・難波｜實際完成：心齋橋 PARCO、法善寺與難波千日前商店街，之後返回飯店。':key==='2026-07-19'?'7/19 梅田・茶屋町｜駿河屋為最後一站，之後直接返回難波飯店。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
 const live=root.querySelector('.v31-live div');
 if(live)live.textContent=`${['2026-07-17','2026-07-18','2026-07-19','2026-07-20'].includes(key)?'實際完成路線':'今日路線'}｜${d.route.join(' → ')}。`;
}
