...
'2026-07-17':{area:'神戶',route:['✓ 生田神社（已完成）','✓ 神戶元町商店街（已完成）','✓ 北野異人館街（已完成）','✓ 神戶牛餐廳（實際用餐、非常滿意）','舊居留地（本次略過）','✓ 神戶塔（已到、現場關閉）','現在：Meriken Park・BE KOBE','Harborland・MOSAIC 大摩天輪','返回 VESSEL INN NAMBA']},
...
function apply(){const root=document.querySelector('#v31');if(!root)return;const key=iso(),d=routes[key];if(!d)return;const box=root.querySelector('.v31-route');if(box)box.innerHTML=d.route.map((x,i)=>`<div class="v31-node"><b>${i+1}. ${x}</b><span>${key==='2026-07-17'&&i<4?'完成':key==='2026-07-17'&&i===4?'略過':key==='2026-07-17'&&i===5?'已到':key==='2026-07-17'&&i===6?'現在':i===0?'起點':i===d.route.length-1?'收尾':'行程'}</span></div>${i<d.route.length-1?'<div class="v31-arrow">→</div>':''}`).join('');const transit=root.querySelector('.v31-transit');if(transit)transit.textContent=`7/${Number(key.slice(-2))} ${d.area}｜神戶塔現場已關閉，現在前往 Meriken Park・BE KOBE，再到 MOSAIC 大摩天輪。`;const live=root.querySelector('.v31-live div');if(live)live.textContent=`目前進度｜${d.route.join(' → ')}。`;}
