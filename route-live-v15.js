...
'2026-07-21':{area:'大阪・宇治',route:['✓ 大阪高島屋','現在：京阪特急往中書島','中書島轉乘京阪宇治線','京阪宇治站','宇治橋・表參道／平等院（依體力）','返回 VESSEL INN NAMBA']},
...
function status(key,i,last){
 ...
 if(key==='2026-07-18'||key==='2026-07-19'||key==='2026-07-20')return '完成';
 if(key==='2026-07-21')return i===0?'完成':i===1?'現在':i===2?'下一站':i===last?'返程':'預定';
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-22'?'7/22 大阪市區｜改排未去過的室內親子路線：Kids Plaza、天神橋筋與大阪くらしの今昔館。':key==='2026-07-21'?'7/21 大阪・宇治｜早上高島屋後搭京阪特急前往中書島，接著轉宇治線。':key==='2026-07-20'?'7/20 心齋橋・難波｜實際完成：心齋橋 PARCO、法善寺、難波千日前商店街與 CoCo壱番屋晚餐。':key==='2026-07-19'?'7/19 梅田・茶屋町｜駿河屋為最後一站，之後直接返回難波飯店。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
 const live=root.querySelector('.v31-live div');
 if(live)live.textContent=`${['2026-07-17','2026-07-18','2026-07-19','2026-07-20'].includes(key)?'實際完成路線':key==='2026-07-21'?'目前路線':'今日路線'}｜${d.route.join(' → ')}。`;
}
