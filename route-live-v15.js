...
'2026-07-17':{area:'神戶',route:['✓ 生田神社（已完成）','✓ 神戶元町商店街（已完成）','現在：北野異人館街','17:00 神戶牛みやび 北野坂店','神戶舊居留地','神戶塔・Meriken Park','Harborland・MOSAIC','返回 VESSEL INN NAMBA']},
...
function apply(){const root=document.querySelector('#v31');if(!root)return;const key=iso(),d=routes[key];if(!d)return;const box=root.querySelector('.v31-route');if(box)box.innerHTML=d.route.map((x,i)=>`<div class="v31-node"><b>${i+1}. ${x}</b><span>${key==='2026-07-17'&&i<2?'完成':key==='2026-07-17'&&i===2?'現在':key==='2026-07-17'&&i===3?'推薦':i===0?'起點':i===d.route.length-1?'收尾':'行程'}</span></div>${i<d.route.length-1?'<div class="v31-arrow">→</div>':''}`).join('');const transit=root.querySelector('.v31-transit');if(transit)transit.textContent=`7/${Number(key.slice(-2))} ${d.area}｜目前在北野異人館；17:00 建議步行至神戶牛みやび 北野坂店。`;const live=root.querySelector('.v31-live div');if(live)live.textContent=`目前進度｜${d.route.join(' → ')}。`;}
