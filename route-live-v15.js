...
function status(key,i,last){
 ...
 if(key==='2026-07-18'||key==='2026-07-19')return '完成';
...
function apply(){
 ...
 if(transit)transit.textContent=key==='2026-07-19'?'7/19 梅田・茶屋町｜駿河屋逛完後直接搭車返回難波，今日行程已完成。':key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：空想機械館、BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;
 const live=root.querySelector('.v31-live div');
 if(live)live.textContent=`${key==='2026-07-17'||key==='2026-07-18'||key==='2026-07-19'?'實際完成路線':'今日路線'}｜${d.route.join(' → ')}。`;
}
