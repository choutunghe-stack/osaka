...
'2026-07-18':{area:'日本橋・難波',route:['✓ 日本橋電器街','✓ 難波 PARKS／T-terrace','✓ BIC CAMERA 難波店','✓ 千日前散步','✓ 道頓堀','✓ 返回 VESSEL INN NAMBA']},
...
function apply(){const root=document.querySelector('#v31');if(!root)return;const key=iso(),d=routes[key];if(!d)return;const box=root.querySelector('.v31-route');if(box)box.innerHTML=d.route.map((x,i)=>`<div class="v31-node"><b>${i+1}. ${x}</b><span>${key==='2026-07-17'?(i===4?'略過':'完成'):key==='2026-07-18'?'完成':i===0?'起點':i===d.route.length-1?'收尾':'行程'}</span></div>${i<d.route.length-1?'<div class="v31-arrow">→</div>':''}`).join('');const transit=root.querySelector('.v31-transit');if(transit)transit.textContent=key==='2026-07-18'?'7/18 日本橋・難波｜實際完成：BIC CAMERA、千日前與道頓堀。':key==='2026-07-17'?'7/17 神戶｜MOSAIC 周邊簡單逛逛後，搭電車返回難波。':`7/${Number(key.slice(-2))} ${d.area}｜依當日安排移動。`;const live=root.querySelector('.v31-live div');if(live)live.textContent=`${key==='2026-07-17'||key==='2026-07-18'?'實際完成路線':'今日路線'}｜${d.route.join(' → ')}。`;}
