...
function apply(){const root=document.querySelector('#v31');if(!root)return;const key=iso(),d=routes[key];if(!d)return;const box=root.querySelector('.v31-route');if(box)box.innerHTML=d.route.map((x,i)=>`<div class="v31-node"><b>${i+1}. ${x}</b><span>${key==='2026-07-17'?(i===4?'略過':'完成'):i===0?'起點':i===d.route.length-1?'收尾':'行程'}</span></div>${i<d.route.length-1?'<div class="v31-arrow">→</div>':''}`).join('');const transit=root.querySelector('.v31-transit');if(transit)transit.textContent=`7/${Number(key.slice(-2))} ${d.area}｜今日神戶行程已完成：MOSAIC 周邊簡單逛逛後，直接搭電車返回難波。`;const live=root.querySelector('.v31-live div');if(live)live.textContent=`實際完成路線｜${d.route.join(' → ')}。`;}
